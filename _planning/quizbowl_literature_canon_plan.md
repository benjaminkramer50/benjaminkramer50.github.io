...
- Creator coverage: `3531 / 4993` rows have some creator signal; `3033 / 4993` have release-ready creator metadata from reviewed, manual, or Wikidata sources.
...
- Default-path coverage: `2715 / 4993` rows currently have non-noisy creator metadata and chronology.
- Remaining chronology backlog: `2249` public rows marked `Unplaced`.
- High-salience release gates: `574` mandatory release-queue rows remain: `37` rank `<=1000` rows, `393` rows with `total_question_count >= 40`, and `517` `qb_major` rows still need resolution by date, merge, rejection, routing, or explicit boundary disposition. These sets overlap.
...
- Known creator-risk constraint: `498` public rows still use `quizbowl_author_answerline` creator inference. These are not release-quality creators unless separately audited or replaced by manual/Wikidata/reviewed metadata.
- Current release-gates artifact: `_planning/quizbowl_lit_canon/quizbowl_lit_release_gates.md` defines finite v1 blockers from machine-readable diagnostics. Current gate counts are `498` creator-risk rows, `113` duplicate-risk rows, `147` boundary-risk rows, and `574` mandatory release-queue rows. The mandatory queue currently has `178` creator-bucket rows, `138` boundary-bucket rows, `475` chronology-bucket rows, and `0` duplicate-bucket rows.
