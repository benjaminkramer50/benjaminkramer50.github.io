...
- Creator coverage: `3534 / 4993` rows have some creator signal; `3036 / 4993` have release-ready creator metadata from reviewed, manual, or Wikidata sources.
- Chronology coverage: `2744 / 4993` public rows have non-`Unplaced` chronology (`46` reviewed canon records, `19` title overrides, `2195` Wikidata overlay rows, `484` manual metadata corrections).
- Default-path coverage: `2718 / 4993` rows currently have non-noisy creator metadata and chronology.
- Remaining chronology backlog: `2249` public rows marked `Unplaced`.
- High-salience release gates: `571` mandatory release-queue rows remain: `35` rank `<=1000` rows, `393` rows with `total_question_count >= 40`, and `517` `qb_major` rows still need resolution by date, merge, rejection, routing, or explicit boundary disposition. These sets overlap.
...
- Known creator-risk constraint: `495` public rows still use `quizbowl_author_answerline` creator inference. These are not release-quality creators unless separately audited or replaced by manual/Wikidata/reviewed metadata.
- Current release-gates artifact: `_planning/quizbowl_lit_canon/quizbowl_lit_release_gates.md` defines finite v1 blockers from machine-readable diagnostics. Current gate counts are `495` creator-risk rows, `113` duplicate-risk rows, `147` boundary-risk rows, and `571` mandatory release-queue rows. The mandatory queue currently has `175` creator-bucket rows, `138` boundary-bucket rows, `475` chronology-bucket rows, and `0` duplicate-bucket rows. There are no remaining rows where creator metadata is the only release blocker.
