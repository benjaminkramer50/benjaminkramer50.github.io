...
- High-salience release gates: `479` mandatory release-queue rows remain: `30` rank `<=1000` rows, `307` rows with `total_question_count >= 40`, and `424` `qb_major` rows still need resolution by date, merge, rejection, routing, or explicit boundary disposition. These sets overlap.
