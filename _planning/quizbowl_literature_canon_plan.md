...
- Creator coverage: `3626 / 4981` rows have some creator signal; `3273 / 4981` have release-ready creator metadata from reviewed, manual, or Wikidata sources.
- Chronology coverage: `3084 / 4981` public rows have non-`Unplaced` chronology (`45` reviewed canon records, `19` title overrides, `2192` Wikidata overlay rows, `828` manual metadata corrections).
- Default-path coverage: `3059 / 4981` rows currently have non-noisy creator metadata and chronology.
- Remaining chronology backlog: `1897` public rows marked `Unplaced`.
- High-salience release gates: `128` mandatory release-queue rows remain after the release-gate alias/rejection, metadata, and boundary-disposition batch. The queue still contains rank `<=1000` rows, rows with `total_question_count >= 40`, and unresolved `qb_major` rows that need resolution by date, merge, rejection, routing, or explicit boundary disposition. These sets overlap.
...
- Known creator-risk constraint: `353` public rows still use `quizbowl_author_answerline` creator inference. These are not release-quality creators unless separately audited or replaced by manual/Wikidata/reviewed metadata.
- Current release-gates artifact: `_planning/quizbowl_lit_canon/quizbowl_lit_release_gates.md` defines finite v1 blockers from machine-readable diagnostics. Current gate counts are `353` creator-risk rows, `111` duplicate-risk rows, `0` boundary-risk rows, and `128` mandatory release-queue rows. The mandatory queue currently has `35` creator-bucket rows, `128` chronology-bucket rows, and `0` duplicate- or boundary-bucket rows. There are no remaining rows where creator metadata is the only release blocker.
...
- The default view is a chronology-ready reading path, currently `3059` rows with non-noisy creator metadata and date, not the full accepted set.
