...
| B | Public-List Purity | active, `2,876` adjudications loaded; first-`1,000` public-row spot check completed; Wikidata audit wave rejected/merged another set of adjacent-domain false positives and title fragments | Top `2,000` audit rows adjudicated, first `1,000` public rows pass spot checks, and the next queue is mostly real boundary cases. | adjudications YAML, rejected TSV, review queue |
| C | Alias And Duplicate Consolidation | complete for broad title-variant pass; `481` manual alias rules added, `478` applied in the latest full rebuild; `13` ambiguous source titles now have `25` author-aware split targets, with `19` public child rows after routing | High-salience duplicate families and whole/subwork boundaries are resolved or explicitly routed. | cluster TSV, adjudications YAML, split-audit TSV |
| D | Classification And Metadata Layer | active, every public row now has provisional classification fields; metadata overlay contains `2891` accepted/corrected rows and contributes `2178` Wikidata chronology rows, `2508` Wikidata creator rows, `263` manual chronology corrections, and `265` manual creator corrections to the public list | Every public row has provisional classification fields, high-salience rows have audited creator/date metadata where available, and unresolved rows stay explicitly marked `unknown`/`Unplaced`. | enriched public YAML, metadata overlay, Wikidata audit TSV |
...
- Public rows: `5062`.
- Creator coverage: `3469 / 5062`.
- Chronology coverage: `2506 / 5062` public rows have non-`Unplaced` chronology (`46` reviewed canon records, `19` title overrides, `2178` Wikidata overlay rows, `263` manual metadata corrections).
- Remaining chronology backlog: `2556` public rows marked `Unplaced`.
...
- Current Wikidata pass: the giant metadata sweep attempted `2310` remaining eligible unplaced rows and initially accepted `815` overlays; four read-only audit chunks then removed or corrected false matches, adjacent-domain spillover, version/edition-level items, creator/date errors, and duplicate/original-language title variants before the full corpus rebuild.
