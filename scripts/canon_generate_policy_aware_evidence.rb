...
works_by_id = work_candidates.to_h { |row| [row.fetch("work_id"), row] }
...
evidence_rows.each do |row|
  source_item = source_items_by_id[row.fetch("source_item_id", "")]
  next unless source_item

  work = works_by_id[source_item.fetch("matched_work_id", "")]
  policy_id = work ? work.fetch("boundary_policy_id", "").to_s : ""
  row["supports_boundary_policy_id"] = policy_id unless policy_id.empty?
end
...
source_items.each do |source_item|
  ...
  work = works_by_id.fetch(source_item.fetch("matched_work_id"))
