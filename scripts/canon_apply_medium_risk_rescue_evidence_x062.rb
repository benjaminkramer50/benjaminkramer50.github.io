...
APPLIED_PATH = File.join(TABLE_DIR, "canon_medium_risk_rescue_evidence_applied.tsv")
REPORT_PATH = File.join(REPORT_DIR, "x_batch_046_x062_medium_risk_rescue_evidence_apply.md")
...
NOTE = "X062 linked for reviewed representative-selection evidence; not complete-work support or cut approval."
...
def write_report(path, rows)
  ...
  File.open(path, "w") do |file|
    file.puts "# X062 Medium-Risk Rescue Evidence Apply"
    file.puts
    file.puts "Status: completed, build-layer only. Public canon path unchanged."
    file.puts
    file.puts "## Purpose"
    file.puts
    file.puts "X062 applies only the medium-risk X061 representative-selection rescue rows. These rows support selected-story or selected-poem representation only; they do not establish complete-work support."
    file.puts
    file.puts "## Output"
    file.puts
    file.puts "- Added `scripts/canon_apply_medium_risk_rescue_evidence_x062.rb`."
    file.puts "- Added `canon_medium_risk_rescue_evidence_applied.tsv`."
    ...
    file.puts
    file.puts "This moves three source-item rows out of the rescue lane into accepted representative-selection evidence. It still leaves the affected cut-side works blocked until complete-work support and cut-side scoring gates are resolved."
    file.puts
...
raise "Expected medium-risk scope rows" if scope_rows.empty?
...
scope_rows.each do |scope|
  ...
  source_item["match_method"] = "x062_current_rescue_scope_review_representative_selection"
  ...
  evidence_id = existing_evidence ? existing_evidence.fetch("evidence_id") : "x062_ev_#{safe_id(source_item_id)}"
  ...
  unless existing_evidence
    evidence_rows << {
      ...
      "notes" => "Applied from X061 medium-risk scope review. Representative-selection evidence only; does not establish complete-work support or approve cuts."
    }
...
applied_rows = applied_rows.map.with_index(1) do |row, index|
  row.merge("applied_id" => "x062_medium_risk_apply_#{index.to_s.rjust(4, "0")}")
end
...
puts "applied or confirmed #{applied_rows.size} medium-risk rescue evidence rows"
