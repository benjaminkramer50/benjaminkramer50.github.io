...
<div class="shelf">
  ...
  <div class="shelf-section" id="shelf-favorites">
    {% if fav_books.size > 0 %}
    <div class="shelf-row">
      {% for item in fav_books limit:12 %}
      <span class="spine" style="--spine-hue: {{ item.title | size | times: 37 | modulo: 360 }}; --spine-width: {{ item.rating | times: 6 | plus: 22 }}px;">
  ...
  <div class="shelf-section" id="shelf-recents">
    <div class="shelf-row">
      {% for item in sorted_books limit:12 %}
      <span class="spine" style="--spine-hue: {{ item.title | size | times: 37 | modulo: 360 }}; --spine-width: {{ item.rating | times: 6 | plus: 22 }}px;">
        <span class="spine-title">{{ item.title }}</span>
      </span>
      {% endfor %}
...
<div class="diary-filters">
  ...
  <input type="text" id="search-filter" placeholder="Search title or author..." aria-label="Search books">
</div>

<div class="diary-table" id="book-diary">
...
(function() {
  ...
  var rows = Array.prototype.slice.call(document.querySelectorAll('#book-diary .diary-row'));
  var monthHeaders = Array.prototype.slice.call(document.querySelectorAll('#book-diary .diary-month-header'));
  ...
  function getFiltered() {
    ...
    return rows.filter(function(row) {
      ...
      if (search) {
        ...
        if (title.indexOf(search) === -1 && author.indexOf(search) === -1) return false;
      }
      return true;
    });
  }
...
<script>new ShelfToggle({ page: 'books' });</script>
